...
cells = [
    ...
    md(
        """
        **Подвывод по этапу 2:** train содержит `7008` строк, test - `1752`; набор обязательных колонок совпадает. Схема теперь задается одним `RAW_FEATURE_SCHEMA`: из него генерируются rename-правила, описания и единицы измерения. Если в будущих данных появятся новые сырые колонки, текущая модель не упадет: они попадут в schema audit как `ignored_by_current_contract`. Чтобы осознанно использовать новый признак, его нужно добавить в schema и pipeline.
    ...
    md(
        """
        Сегменты готовы. Теперь считаю для каждого среза одинаковые метрики baseline и финальной модели: RMSE, MAE, средний фактический спрос и число отрицательных прогнозов.
        """
    ),
    code(
        r'''
        ...
        show_markdown(
            ...
            **Расчетные итоги сегментного аудита**
            ...
            - Самый сильный выигрыш: `{best_segment["segment_group"]}` /
              ...
              улучшение `{best_segment["RMSE_delta"]:.2f}` проката/час
            ...
            - Самый слабый сегмент: `{weakest_segment["segment_group"]}` /
              ...
              изменение RMSE `{weakest_segment["RMSE_delta"]:.2f}` проката/час.
            """
        )
        '''
    ),
    md(
        """
        Таблица выше отсортирована по выигрышу RMSE. Для чтения оставляю график только по самым заметным срезам: слева величина выигрыша, справа абсолютный RMSE baseline и финальной модели в тех же сегментах.
        """
    ),
    code(
        r'''
        ...
        final_calculated_summary = "\n".join(
            [
                ...
                (
                    ...
                    f"`RMSE -{rmse_abs_improvement:.2f}` проката/час "
                ...
                (
                    ...
                    f"(`RMSE -{best_segment['RMSE_delta']:.2f}` проката/час)."
                ),
